...
class BitfinexExchangeAPI {
     ...
     */
    static product(coinbaseProProduct) {
        return BitfinexCommon_1.PRODUCT_MAP[coinbaseProProduct] || coinbaseProProduct;
    }
    ...
    loadProducts() {
        return request.get(`${API_V1}/symbols_details`)
            ...
            return Promise.reject(new errors_1.CBPTTError('Error loading products from Bitfinex', err));
        });
    }
    loadMidMarketPrice(coinbaseProProduct) {
        return this.loadTicker(coinbaseProProduct).then((ticker) => {
            return ticker.bid.plus(ticker.ask).times(0.5);
        });
    }
    loadOrderbook(coinbaseProProduct) {
        ...
        return request.get(`${API_V1}/book/${product}`)
            ...
            return Promise.resolve(this.convertBitfinexBookToCoinbaseProBook(res.body));
        }).catch((err) => {
            return Promise.reject(new errors_1.CBPTTError(`Error loading ${coinbaseProProduct} order book from Bitfinex`, err));
        });
    }
    loadTicker(coinbaseProProduct) {
        const product = BitfinexExchangeAPI.product(coinbaseProProduct);
        return request.get(`${API_V1}/pubticker/${product}`)
            ...
            return Promise.reject(new errors_1.CBPTTError(`Error loading ${coinbaseProProduct} ticker from Bitfinex`, err));
        });
    }
    checkAuth() {
        return new Promise((resolve, reject) => {
            if (this.auth === null) {
                return reject(new errors_1.CBPTTError('You cannot make authenticated requests if a ExchangeAuthConfig object was not provided to the BitfinexExchangeAPI constructor'));
            }
    ...
    cancelAllOrders(_coinbaseProProduct) {
        return this.checkAuth().then((auth) => {
    ...
    loadAllOrders(_coinbaseProProduct) {
        return this.checkAuth().then((auth) => {
    ...
    loadCandles(_options) {
        return Promise.reject(new errors_1.CBPTTError('Not implemented'));
    }
    // -------------------------- Transfer methods -------------------------------------------------
    ...
    // -------------------------- Helper methods -------------------------------------------------
    convertBitfinexBookToCoinbaseProBook(bfBook) {
