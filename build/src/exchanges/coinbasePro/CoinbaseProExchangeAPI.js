...
const PublicExchangeAPI_1 = require("../PublicExchangeAPI");
...
exports.COINBASE_PRO_API_URL = 'https://api.pro.coinbase.com';
class CoinbaseProExchangeAPI {
    constructor(options) {
        this.owner = 'Coinbase Pro';
        this._apiURL = options.apiUrl || exports.COINBASE_PRO_API_URL;
        this.auth = options.auth;
        this.logger = options.logger;
        if (this.auth) {
            this.authClient = new coinbase_pro_1.AuthenticatedClient(this.auth.key, this.auth.secret, this.auth.passphrase, this._apiURL);
        }
        this.publicClient = new coinbase_pro_1.PublicClient(this._apiURL);
    }
    get apiURL() {
        return this._apiURL;
    }
    ...
    loadProducts() {
        return this.publicClient.getProducts()
            ...
            return Promise.reject(new errors_1.HTTPError('Error loading products from Coinbase Pro', errors_1.extractResponse(err.response)));
        });
    }
    loadMidMarketPrice(product) {
        return this.loadTicker(product).then((ticker) => {
            if (!ticker || !ticker.bid || !ticker.ask) {
                const err = new errors_1.HTTPError(`Loading midmarket price for ${product} failed because ticker data was incomplete or unavailable`, { status: 200, body: ticker });
    ...
    loadFullOrderbook(product) {
        return this.loadCoinbaseProOrderbook({ product: product, level: 3 }).then((body) => {
            return this.buildBook(body);
        });
    }
    loadCoinbaseProOrderbook(options) {
        ...
        return this.publicClient.getProductOrderBook(product, params)
            ...
            if (!(orders.bids && orders.asks)) {
                return Promise.reject(new errors_1.HTTPError(`Error loading ${product} orderbook from Coinbase Pro`, { status: 200, body: orders }));
            }
            return orders;
        }).catch((err) => {
            return Promise.reject(new errors_1.HTTPError(`Error loading ${product} orderbook from Coinbase Pro`, errors_1.extractResponse(err.response)));
        });
    }
    loadTicker(product) {
        return this.publicClient.getProductTicker(product)
            ...
            return Promise.reject(new errors_1.HTTPError(`Error loading ${product} ticker from Coinbase Pro`, errors_1.extractResponse(err.response)));
        });
    }
    loadCandles(options) {
        ...
        return this.publicClient.getProductHistoricRates(product, {
            granularity: PublicExchangeAPI_1.IntervalInMS[options.interval] * 0.001,
            ...
            return data.map((d) => {
                return {
                    ...
                    open: types_1.Big(d[3]),
                    close: types_1.Big(d[4]),
                    volume: types_1.Big(d[5])
                };
            });
        }).catch((err) => {
            return Promise.reject(new errors_1.HTTPError(`Error loading ${product} candles from Coinbase Pro`, errors_1.extractResponse(err.response)));
        });
    }
    aggregateBook(body) {
    ...
    placeOrder(order) {
        if (!this.authClient) {
            return Promise.reject(new errors_1.CBPTTError('No authentication details were given for this API'));
        }
        let coinbaseProOrder;
        switch (order.orderType) {
            ...
            case 'stop':
                coinbaseProOrder = {
                    ...
                    client_oid: order.clientId,
                    stp: order.extra && order.extra.stp
                }; // Override for incomplete definition in Coinbase Pro lib
                break;
            default:
                return Promise.reject(new errors_1.CBPTTError(`Invalid order type: ${order.type}`));
        }
        return this.authClient.placeOrder(coinbaseProOrder).then((result) => {
            // Check for error
            if (result.status === 'rejected' || result.message) {
                return Promise.reject(new errors_1.APIError(`Placing order on ${order.productId} failed`, undefined, result));
            }
            return Promise.resolve(CoinbaseProOrderResultToOrder(result));
        }, (err) => {
    ...
    cancelAllOrders(product) {
        ...
        return this.handleResponse(apiCall, options);
    }
    loadOrder(id) {
        if (!this.authClient) {
            return Promise.reject(new errors_1.CBPTTError('No authentication details were given for this API'));
        ...
        return this.authClient.getOrder(id).then((order) => {
            return CoinbaseProOrderInfoToOrder(order);
        }).catch((err) => {
            return Promise.reject(new errors_1.HTTPError('Error loading order details on Coinbase Pro', errors_1.extractResponse(err.response)));
        });
    }
    loadAllOrders(product) {
        ...
        const loop = (after) => {
            return self.loadNextOrders(product, after).then((result) => {
                const liveOrders = result.orders.map(CoinbaseProOrderInfoToOrder);
    ...
    loadBalances() {
        ...
        return this.authClient.getAccounts().then((accounts) => {
            ...
            return Promise.reject(new errors_1.HTTPError('Error loading account balances on Coinbase Pro', errors_1.extractResponse(err.response)));
        });
    }
    ...
    handleResponse(req, _meta) {
        return req.then((res) => {
            if (res.status >= 200 && res.status < 300) {
                return res.body;
            }
            const err = new errors_1.HTTPError(`Error handling Coinbase Pro request for ${req.url}`, res);
            return Promise.reject(err);
        }).catch((err) => {
            const error = new errors_1.CBPTTError('A Coinbase Pro API request failed.', err); // TODO add req url here
    ...
    checkAuth() {
        return new Promise((resolve, reject) => {
            if (!this.auth) {
                return reject(new errors_1.CBPTTError('You cannot make authenticated requests if a CoinbaseProAuthConfig object was not provided to the CoinbaseProExchangeAPI constructor'));
            }
            if (!(this.auth.key && this.auth.secret && this.auth.passphrase)) {
                return reject(new errors_1.CBPTTError('You cannot make authenticated requests without providing all API credentials'));
            }
            return resolve();
        });
    }
    ...
    requestCryptoAddress(cur) {
        return this.loadCoinbaseAccount(cur, false).then((account) => {
            const id = account.id;
            if (!id) {
                return Promise.reject(new errors_1.CBPTTError('Coinbase account does not have an ID'));
            }
            ...
            if (!validResult) {
                return Promise.reject(new errors_1.CBPTTError(`Could not obtain a valid crypto address for${cur}`));
            }
    ...
    requestTransfer(req) {
        ...
        return Promise.reject('Coinbase Pro does not support multiple accounts.');
    }
    requestWithdrawal(req) {
        ...
        return this.authClient.withdrawCrypto(params).catch((err) => {
            return Promise.reject(new errors_1.HTTPError(`Error on Coinbase Pro withdrawal request`, errors_1.extractResponse(err.response)));
        });
    }
    // ------------------------------ Coinbase Pro-specific public Methods ------------------------------------------------//
    ...
    loadCoinbaseAccount(currency, force) {
        return this.loadCoinbaseAccounts(force).then((accounts) => {
            ...
            return Promise.reject(new errors_1.CBPTTError(`No Coinbase account for ${currency} exists`));
        });
    }
    buildBook(body) {
        const book = new BookBuilder_1.BookBuilder(this.logger);
...
exports.CoinbaseProExchangeAPI = CoinbaseProExchangeAPI;
function CoinbaseProOrderResultToOrder(order) {
    let size;
    let price;
    if (+order.size > 0) {
...
function CoinbaseProOrderInfoToOrder(order) {
    let size;
    let price;
    if (+order.size > 0) {
        size = types_1.Big(order.size);
...
//# sourceMappingURL=CoinbaseProExchangeAPI.js.map