...
class BitmexMarketFeed extends ExchangeFeed_1.ExchangeFeed {
    ...
    handleSnapshot(snapshot) {
        ...
        const initOrderIdMap = {};
        this.orderIdMap = snapshot.data.reduce((acc, { id, price }) => (Object.assign({}, acc, { [id]: price })), initOrderIdMap);
        const mapLevelUpdates = ({ price, size, side }) => Orderbook_1.PriceLevelFactory(price, size, sides_1.Side(side));
        ...
        const priceDataToLvl3 = ({ price, size, side, id }) => ({
            ...
            side: sides_1.Side(side),
            id: id.toString(),
        });
        const initOrderPool = {};
        const orderPool = snapshot.data.reduce((acc, pd) => (Object.assign({}, acc, { [pd.id.toString()]: priceDataToLvl3(pd) })), initOrderPool);
        const snapshotMsg = {
            time: new Date(),
            sequence: 0,
            type: 'snapshot',
    ...
    handleTrade(trades) {
        trades.data.forEach((trade) => {
            const message = {
                ...
                side: sides_1.Side(trade.side),
            };
            this.push(message);
        });
    }
    handleSubscriptionSuccess(_successMsg) {
        // TODO
    }
}
