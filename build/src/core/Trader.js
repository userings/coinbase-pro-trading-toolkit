...
function isCancelMyOrdersRequestMessage(msg) {
    return msg.type === 'cancelMyOrders';
}
exports.isCancelMyOrdersRequestMessage = isCancelMyOrdersRequestMessage;
function isCancelAllOrdersRequestMessage(msg) {
    return msg.type === 'cancelAllOrders';
}
exports.isCancelAllOrdersRequestMessage = isCancelAllOrdersRequestMessage;
function isTraderStreamMessage(msg) {
    return Messages_1.isStreamMessage(msg) || msg.type === 'cancelMyOrders' || msg.type === 'cancelAllOrders';
}
exports.isTraderStreamMessage = isTraderStreamMessage;
/**
...
class Trader extends stream_1.Writable {
    ...
    cancelAllOrders(productId) {
        return (productId ? this.api.cancelAllOrders(productId) : this.api.cancelAllOrders()).then((ids) => {
            this.myBook.clear();
    ...
    checkState() {
        return this.api.loadAllOrders(this.productId).then((actualOrders) => {
            ...
            return OrderbookDiff_1.OrderbookDiff.compareByOrder(this.myBook, book);
        });
    }
    executeMessage(msg) {
    ...
    handleTradeExecutedMessage(msg) {
        ...
        if (!order) {
            ...
            this.emit('Trader.outOfSyncWarning', `Traded order ${msg.orderId} not in my book`);
            return;
        }
        let newSize;
        if (msg.tradeSize) {
