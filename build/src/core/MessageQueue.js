...
class MessageQueue extends stream_1.Duplex {
     ...
     */
    addMessage(message) {
        if (Messages_1.isSnapshotMessage(message) && this.waitForSnapshot) {
    ...
    _write(inputMessage, _encoding, callback) {
    ...
    pop() {
        ...
        if (node) {
            ...
            if (node.sequence && expectedSequence < 0 && this.waitForSnapshot && !Messages_1.isSnapshotMessage(node)) {
                return null;
            }
    ...
    defaultMessageHandler(msg) {
        ...
        if (Messages_1.isSequencedMessage(msg)) {
            this.addMessage(msg);
        }
        return true;
    }
